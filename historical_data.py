...
market = 'BTCUSDT'
tick_interval = '1m'
stock_exchange = 'Binance SPOT'

# todo: file import
r = requests.get(BASE_URL)
open('tmp/BTCUSDT-1m-2021-03-17.zip', 'wb').write(r.content)

# todo: zip unpack
with zipfile.ZipFile("tmp/BTCUSDT-1m-2021-03-17.zip", "r") as zip_ref:
    zip_ref.extractall("tmp")

# insert file into DBMS
csv_data = csv.reader(open('tmp/BTCUSDT-1m-2021-03-17.csv'))
...
cursor.execute(
    "DELETE FROM " + db_schema_name + "." + db_table_name + " where open_time >= %s and open_time <= %s and tick_interval = %s and market = %s and stock_exchange = %s",
    (open_time_min, open_time_max, tick_interval, market, stock_exchange))
print("delete done")
