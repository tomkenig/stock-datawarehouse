...
cursor.execute("SELECT download_settings_id, market, tick_interval, stock_type, stock_exchange, api_range_to_overwrite, download_interval_sec FROM " + db_schema_name + "." + db_settings_table_name + " WHERE coalesce(next_download_ux_timestamp, 0) <= " + ts + " order by next_download_ux_timestamp asc limit 1")
...
print(timestamp_delete)

# todo: params 1,1,1 (get them from db
# insert and overwrite fresh data
def fn_insert_overwrite_last_data():
    try:
        cursor.execute("DELETE FROM " + db_schema_name+"."+db_table_name +" where open_time >= %s and tick_interval = %s and market = %s and stock_type = %s and stock_exchange = %s", (timestamp_delete, market, tick_interval, stock_type, stock_exchange))
        print("delete done")
        for i in short_data:
...
def fn_updatesettings_queue():
    try:
        cursor.execute("UPDATE " + db_schema_name + "." + db_settings_table_name + " SET last_download_ux_timestamp = %s, next_download_ux_timestamp = %s where download_settings_id = %s", (ts, str(int(ts) + download_interval_sec), download_settings_id))
        print("update done")
        cnxn.commit()
        cursor.close()
        cnxn.close()
